...
- **Events calendar:** `events.html` embeds the Canva calendar from `https://canva.link/deareleanoreventscalendar` using Canva’s `view?embed` URL. The homepage menu and hero button open the events page in a new tab, and the page footer link opens the Canva calendar directly.
- **Ticketing:** The ticketing section embeds Dear Eleanor’s Posh page at `https://posh.vip/g/dear-eleanor` and includes a direct “Open tickets” link as a fallback.
