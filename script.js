...
// Posh switches to one event per row at 800px. Preserve its two-column
// viewport inside our responsive frame; the direct link offers full-size checkout.
const poshFrame = document.querySelector('.posh-embed-frame');
if (poshFrame) {
  const poshEmbed = poshFrame.querySelector('iframe');
  const fitPoshEmbed = () => {
    const width = poshFrame.clientWidth;
    if (!width) return;
    const embedWidth = Math.max(820, width);
    const scale = width / embedWidth;
    poshEmbed.style.width = `${embedWidth}px`;
    poshEmbed.style.height = `${poshFrame.clientHeight / scale}px`;
    poshEmbed.style.transform = `scale(${scale})`;
    poshEmbed.style.transformOrigin = 'top left';
  };
  new ResizeObserver(fitPoshEmbed).observe(poshFrame);
  fitPoshEmbed();
}
...
if (bloomFrameMount) {
  ...
  window.addEventListener('message', (event) => {
    ...
    bloomFrame.style.height = `${Math.min(Math.max(nextHeight + 8, 360), 20000)}px`;
  });
}
...
if (ambientPointerMedia.matches && !reducedMotion.matches) {
  ...
  window.addEventListener('pointermove', (event) => {
    ...
    const isLightSection = target?.closest('.pitch-panel, .contact-section');
