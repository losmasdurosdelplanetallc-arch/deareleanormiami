...
const mailchimpFrame = document.querySelector('iframe[name="mailchimp-subscribe-frame"]');

if (mailingForm && formMessage) {
  let mailchimpSubmitted = false;

  mailingForm.addEventListener('submit', (event) => {
    const submitButton = mailingForm.querySelector('button[type="submit"]');

    if (!mailingForm.reportValidity()) {
      event.preventDefault();
      return;
    }

    mailchimpSubmitted = true;
    submitButton.disabled = true;
    formMessage.textContent = 'Sending directly to Mailchimp...';
  });

  mailchimpFrame?.addEventListener('load', () => {
    if (!mailchimpSubmitted) return;
    mailchimpSubmitted = false;
    mailingForm.querySelector('button[type="submit"]').disabled = false;
    formMessage.textContent = 'Sent to Mailchimp. If confirmation is enabled, check your inbox or spam. ♡';
    mailingForm.reset();
  });
}
