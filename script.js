...
const cursorMedia = window.matchMedia('(pointer: fine) and (min-width: 901px)');
...
if (cursorMedia.matches && !reducedMotion.matches) {
  let pointerX = -500;
  let pointerY = -500;
  let glowX = -500;
  let glowY = -500;
  ...
  const drawCursor = () => {
    ...
    brandCursor.style.transform = `translate3d(${pointerX}px, ${pointerY}px, 0) translate(-50%, -50%)`;
  ...
  window.addEventListener('pointermove', (event) => {
    ...
    brandCursor.classList.add('cursor-visible');
    cursorAmbient.classList.add('cursor-visible');

    const target = event.target instanceof Element ? event.target : null;
    const isInteractive = target?.closest('a, button, input, textarea, select, iframe');
    const isLightSection = target?.closest('.about-section, .pitch-panel, .creative-bridge, .contact-section');
    brandCursor.classList.toggle('cursor-over', Boolean(isInteractive));
    cursorAmbient.classList.toggle('on-light', Boolean(isLightSection));
  }, { passive: true });

  window.addEventListener('pointerdown', () => brandCursor.classList.add('cursor-down'));
  window.addEventListener('pointerup', () => brandCursor.classList.remove('cursor-down'));
  document.documentElement.addEventListener('mouseleave', () => {
    brandCursor.classList.remove('cursor-visible');
