...
if (mailchimpLiteForm && mailchimpMessage) {
  mailchimpLiteForm.addEventListener('submit', (event) => {
    ...
    const timeout = window.setTimeout(() => {
      mailchimpMessage.textContent = 'Hmm — try again, or open the signup link below.';
      cleanup();
    }, 10000);
...
function bindTwitchEvents(player) {
  ...
  if (window.Twitch.Player.PLAY) {
    player.addEventListener(window.Twitch.Player.PLAY, confirmRadioPlayback);
  }

  if (window.Twitch.Player.PLAYING) {
    player.addEventListener(window.Twitch.Player.PLAYING, confirmRadioPlayback);
  }
...
async function preloadOrAutoplaySound() {
  ...
  try {
    ...
    player.setMuted(false);
    player.setVolume(0.62);
    soundStatus.textContent = 'Miami Community Radio is ready. Tap for radio.';
  } catch (error) {
    startFallbackMusic();
  }
}
...
async function startRadioStream() {
  if (!canUseTwitchPlayer()) return false;

  try {
    const player = await getTwitchPlayer();
    ambientAudio.pause();
    radioPlayConfirmedAt = 0;
    player.setMuted(false);
    player.setVolume(0.72);
    player.play();
    await wait(2800);
    return Date.now() - radioPlayConfirmedAt < 5000;
  } catch (error) {
    return false;
  }
}
...
async function startMusic() {
  ...
  const radioStarted = await startRadioStream();
  if (radioStarted) {
    disarmMusicUnlock();
    return true;
  }
